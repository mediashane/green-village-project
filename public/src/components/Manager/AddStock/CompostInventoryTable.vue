<template>
  <v-data-table
    ...
    :headers="headers"
    :items="$firestore.stock.filter(x => x.type === 'Compost')"
...
export default {
  ...
  created () {
    console.log(this.$firestore.drivers)
    let stockWeights = []
    this.$firestore.stock.filter(x => x.type === 'Compost').map(item => stockWeights.push(parseInt(item.weight)))
